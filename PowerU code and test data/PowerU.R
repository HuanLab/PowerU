...
for (i in 1:nrow(calibrated_table)) {
  ...
  if(n1 > 0.1 & n2 > 0.1){
    ...
    if(f > 0.05){calibrated_table$p.value[i] = t.test(g1, g2, conf.level = 0.95, var.equal = T)$p.value
    }else{calibrated_table$p.value[i] = t.test(g1, g2, conf.level = 0.95, var.equal = F)$p.value}
  }  else{
